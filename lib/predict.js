...
// Empirically calibrated against DU's CUET-era cutoff movements
// (typical year-on-year drift of 1–3% for top programs):
//   1% growth in category pool → ~0.08–0.12% upward drift in cutoff.
// Elite courses are LESS elastic, not more — they are already near
// saturation, so extra demand can't push them up linearly.
// =====================================================================
function competitionDelta(category, baseCutoff, formulaMax, isElite) {
  ...
  const growthPct     = blendedGrowth - 1.0;                          // e.g. 0.064 = +6.4%

  // Elasticity: elite tier is LESS elastic (already near ceiling).
  // Calibrated so a typical 6% growth shifts an elite cutoff by ~0.7%
  // and a general cutoff by ~0.5% — matching DU 2024→2025 observed drift.
  const elasticity = isElite ? 0.12 : 0.08;
...
// Empirically calibrated to add 5–12 marks to top-elite cutoffs,
// not 30–60.
// =====================================================================
function topDensityPremium(formulaMax, isElite, eliteIntensity) {
  ...
  // Calibrated coefficient: 0.05 means a 20% density growth → 1% cutoff lift
  // at peak elite intensity. Scales linearly down to 0 at the elite floor.
...
export function projectCutoff(cutoff2025, programInfo, category, usedCodes) {
  ...
  // --- (g) Sum it all up ------------------------------------------------
  let mostLikely = baseProjected + dComp + dDensity + dScarcity;

  // --- (g.1) PHYSICAL REALISM CAP ---------------------------------------
  ...
  // Cap at 95% of the 2026 formula max so projections stay physically
  // plausible. (Top-20 students in 2026 averaged 1182/1250 = 94.6%.)
  const physicalCeiling = fShift.max26 * 0.95;
  if (mostLikely > physicalCeiling) {
    factors.push({
      id: 'CAP',
      name: 'Physical realism ceiling applied',
      value: `Capped at 95% of max (${physicalCeiling.toFixed(0)})`,
      contribution: physicalCeiling - mostLikely,
      note: `Raw projection was ${mostLikely.toFixed(1)} — but the historical top-20 CUET average is ~94.6% of max, so cutoffs above 95% are statistically implausible.`
    });
    mostLikely = physicalCeiling;
  }
  ...
  // Clamp into the legal range [0, physicalCeiling]
  const clamp = v => Math.max(0, Math.min(physicalCeiling, v));
...
export function admissionProbability(studentScore, projection, seatsInCategory) {
  ...
  // Logistic steepness k: calibrated so that
  //   P(margin = +σ)  ≈ 73%   (clearly above cutoff = strong chance)
  //   P(margin =  0)  = 50%   (right on cutoff = coin flip)
  //   P(margin = −σ)  ≈ 27%   (just below cutoff = real chance via round drift / withdrawals)
  // Solving 1/(1+exp(-k·σ)) = 0.73 → k·σ ≈ 1.0.
  // So k = 1 / σ. This gives realistic probabilities for borderline scores.
  let k = 1.0 / sigma;
  ...
  // ============================================================
  // TOP-TIER RANK BOOST (the "elite ceiling" correction)
  // When the student's composite is in the top ~1% of the theoretical
  // distribution AND clearly above the projected cutoff, the binding
  // constraint stops being the cutoff and becomes "how many competitors
  // outrank you?". At 94%+ of max, there are simply not enough higher
  // scorers to fill seats anywhere. This corrects an under-prediction
  // bias for very high scores.
  // ============================================================
  if (projection.formulaMax26 && studentScore > 0) {
    const studentPctOfMax = studentScore / projection.formulaMax26;
    // Only kicks in for genuinely elite scores AND when above cutoff
    if (studentPctOfMax >= 0.90 && margin > 0) {
      // Boost scales by how far into "top tier" the student is:
      //   0.90 of max → modest boost
      //   0.93+ of max → strong boost
      //   0.95+ of max → near-certain
      // Floor the probability at this elite-tier minimum
      let eliteFloor;
      ...
      // Only apply the floor if margin is meaningful (≥ +3 marks)
      // — protects against false elites who barely scraped above
      if (margin >= 3) {
