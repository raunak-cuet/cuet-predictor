...
export function runEngine({ scores, category }) {
  ...
  for (const prog of PROGRAMS) {
    ...
    let seatsInCat = null;
    if (prog.seats && typeof prog.seats[cat] === 'number') seatsInCat = prog.seats[cat];
    // BFIA/BMS/BBE entrance seats live under entrance map already.
    // For merit programs, leave null (engine handles gracefully).

    // For probability, use UR seats if category has 0 reserved seats.
    // Guard: only use UR as proxy when UR itself is a positive number
    // (entrance-based programs may have UR=0 too — don't create a false proxy).
    const urSeats = prog.seats?.['UR'];
    const seatsForProb = (seatsInCat === 0 && typeof urSeats === 'number' && urSeats > 0)
      ? urSeats
      : seatsInCat;
    const probability = admissionProbability(comp.composite, projection, seatsForProb);
