...
export function computeComposite(formula, scores) {
  ...
  switch (fType) {
    ...
    // -------- B.Com (Hons.) / B.Com Prog. : 1 Lang + 3 best Commerce (Acc/BSt/Eco/Math) --------
    case 'BCOM_HONS':
    case 'BCOM_PROG': {
      lang = pushLang(); if (!lang) return ineligible('No language taken');
      const k3 = bestK(scores, COMMERCE_CODES, 3);
      if (k3.length < 3) return ineligible('Need 3 of: Accountancy, Business Studies, Economics, Mathematics');
      // DU requires at least Math or Acc as one of them — relaxed/auto-satisfied by best-of-3 in most realistic cases.
      k3.forEach(x => used.push({ code: x.code, score: x.score, role: 'Best Commerce' }));
      return sum(used);
    }
